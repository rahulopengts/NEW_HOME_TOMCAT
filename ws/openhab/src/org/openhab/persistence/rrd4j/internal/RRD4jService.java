			...
			try {
					...
					if(filter.getOrdering()==Ordering.DESCENDING && filter.getPageSize()==1 && filter.getPageNumber()==0) {
						if(filter.getEndDate()==null) {
							...
							if(!Double.isNaN(lastValue)) {
								...
								System.out.println("\n***RRD4JService-query->Item->InitValue"+itemName+"->State->"+mapToState(lastValue, itemName));
				...
				for(double value : result.getValues(DATASOURCE_STATE)) {
					if(!Double.isNaN(value)) {
						...
						System.out.println("\n***RRD4JService-query->Item->InitValue"+itemName+"->State->"+mapToState(value, itemName));
						items.add(rrd4jItem);
					}
					ts += step;
				}
