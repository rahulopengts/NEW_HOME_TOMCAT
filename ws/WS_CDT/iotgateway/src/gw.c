...
void setEventId(int eventIdValue){
...
int writePortTest(unsigned char *buff_p, unsigned int cnt)
{
	printf("\n Writing to port values \n");
}
...
int writePort(unsigned char *buff_p, unsigned int cnt)
{
   ...
   while (bytesLeft > 0)
   {
      ...
      printf("\n Wrote to port and response is %i ",rc);
      if (rc <= 0)
          return -1;
      else
      {
...
{
	if(myhubverbose){
		printf("\n Going to read in GW_processRcvdMsg \n");
	}
   int rc = 0;
...
int GW_readSerIntf(int expMsgType, int tmoSecs)
{
   ...
   do
   {
      ...
      {
    	  if(myhubverbose){
    		  printf("\n Reading port in GW_readSerIntf with expMsgType type as a..... %i \n",expMsgType);
    	  }
          rc = readPort(serRxBuff + off, readLen);

          if(myhubverbose){
    		  printf("\n Read port in GW_readSerIntf with expMsgType type as %i \n",expMsgType);
    	  }
...
//		   {
//			   if(myhubverbose){
//				   printf("\n Handling Count Message \n");
//			   }
//
//			   GW_readSerIntf(LPWMN_GW_MSG_TYPE_NODE_CNT_REQ, 0);
//
//			   if(myhubverbose){
//				   printf("\n Handled Count Message Read \n");
//			   }
//		   }
       default:
            break;
...
void GW_handleNwkDataTraffic(int expShortAddr)
{
    ...
    {
    	if(myhubverbose){
    		printf("\n Global eventId Variable is %i \n",eventId);
    	}
    	if(eventId==1){
    		GW_getNodeCntReqHndlr();
    	}
       int rc;
       ...
       if(myhubverbose){
    	   printf("\n Read From Port Is %s ",serInputBuff);
       }
       if (rc != readLen)
       {
           printf("\nreadPort() failed !! ................. \n");
       ...
       switch (totReadLen)
       {
          ...
          default:
               {
                   if(myhubverbose){
                	   printf("\n Going For Default Processing for type %i \n",currMsgType);
                   }
...
//--------------------------------------------------MyHub Changes Starts Here-------------------------------------------------------

void handleInboundMessage(int sensorOpVal,int sensorNodeIdVal,char macId){
	printf("\n Going to read the port now \n");
	int sensorDataLength	=	19;
	char charSensorDataValue[19];
	char *topic	=	"r";

	char* inboundMessageCurrent_1	= malloc( strlen(INBOUND_MESSAGE_TEMPLATE)+1 );
	strcpy(inboundMessageCurrent_1,INBOUND_MESSAGE_TEMPLATE);
	createInboundMessage(inboundMessageCurrent_1,sensorNodeIdVal,sensorOpVal);
