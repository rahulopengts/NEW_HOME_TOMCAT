...
#define BUILD_TIMESTAMP "Thu Apr  7 08:24:04 IST 2016"
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	int rc = MQTTASYNC_SUCCESS;
	MQTTAsync_queuedCommand* conn;
	printf("\n MQTTAsync_connect Going to Connect-> 1 \n");
	FUNC_ENTRY;
	if (options == NULL)
	{
		printf("\n MQTTAsync_connect Going to Connect-> NULL OPTION \n ");
		rc = MQTTASYNC_NULL_PARAMETER;
		goto exit;
	}
	...
	{
		printf("\n MQTTAsync_connect Going to Connect-> 2 ");
		rc = MQTTASYNC_BAD_STRUCTURE;
		goto exit;
	}
	if (options->will) /* check validity of will options structure */
	{
		printf("\n MQTTAsync_connect Going to Connect-> 3 ");
		if (strncmp(options->will->struct_id, "MQTW", 4) != 0 || options->will->struct_version != 0)
		{
			printf("\n MQTTAsync_connect Going to Connect-> 4 ");
			rc = MQTTASYNC_BAD_STRUCTURE;
			goto exit;
		}
		if (options->will->qos < 0 || options->will->qos > 2)
		{
			printf("\n MQTTAsync_connect Going to Connect-> 5 ");
	...
	{
		printf("\n MQTTAsync_connect Going to Connect-> 6 ");
		rc = MQTTASYNC_BAD_UTF8_STRING;
		goto exit;
	}
	printf("\n MQTTAsync_connect Going to Connect-> 7 ");
	...
	printf("\n MQTTAsync_connect Going to Connect-> 9 ");
													 ...
													 MQTTAsync_responseOptions* response)
{
	...
	if (message == NULL)
	{
		printf("MQTTAsync_sendMessage-->Message Is Null\n");
		rc = MQTTASYNC_NULL_PARAMETER;
		goto exit;
	}
	if (strncmp(message->struct_id, "MQTM", 4) != 0 || message->struct_version != 0)
	{
		printf("MQTTAsync_sendMessage-->strncmp is bad--MQTTASYNC_BAD_STRUCTURE \n");
		rc = MQTTASYNC_BAD_STRUCTURE;
		goto exit;
	}

	printf("MQTTAsync_sendMessage--> 2 \n");
	rc = MQTTAsync_send(handle, destinationName, message->payloadlen, message->payload,
								message->qos, message->retained, response);
exit:
	FUNC_EXIT_RC(rc);
	printf("MQTTAsync_sendMessage--> Return\n");
	return rc;
}
